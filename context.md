...
- **Raw Schema Injection 적용**: `image/route.ts`와 `video/route.ts`의 스키마 추출 로직을 전면 수정하여, OpenAPI의 `schemaObj.properties` 객체 자체를 필터링 없이 그대로 `JSON.stringify()`하여 LLM에 주입하도록 변경 (`schemaPropertiesString`).
- **분석 프롬프트 최적화**: `POST_IMAGE_PRICE_ANALYSIS_PROMPT.ts` 및 `POST_VIDEO_PRICE_ANALYSIS_PROMPT.ts` 업데이트. LLM이 `<schema_properties>` 원본 JSON 문자열을 파싱하고, `enum`, `default`, `min/max` 등의 한계 조건을 포괄적으로 추론하도록(comprehensively infer) 지침 보강.
- **비디오 시간 간격 정책 (Rule 3.3) 검증**: 최대 2초의 Duration Gap 허용 정책(예: 4, 6, 8) 유지 확정. FFmpeg 후처리 로직(`postProcessedVideo`)의 `isCutting` 방어 코드가 목표 시간보다 길게 생성된 영상에 대해 배속 조정(Slow-motion) 없이 안전하게 자르기(Cutting)만 수행함을 검증 완료.
...
1. **AI 모델 DB 동기화 파이프라인 E2E 테스트**:
   - 신규 Raw Schema Injection이 적용된 이미지/비디오 스크래핑 엔드포인트 로컬 구동 및 결과물(DB 적재) 정상 여부 재검증.
