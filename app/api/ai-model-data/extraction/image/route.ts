...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const rawJsonPath = path.join(process.cwd(), 'raw_input_data_image.json');
        const rawCsvPath = path.join(process.cwd(), 'raw_input_data_image.csv');
        const dbCsvPath = path.join(process.cwd(), 'db_ready_data_image.csv');

        // JSON 저장 (T2I 포함 전체 데이터)
        fs.writeFileSync(rawJsonPath, JSON.stringify(newAIModelMetadataList, null, 2), { encoding: 'utf-8' });
        fs.chmodSync(rawJsonPath, 0o666);

        // CSV 저장 (헤더 및 데이터, T2I 포함)
        const csvHeader = 'endpointId,displayName,description,matchedSchemaName,schemaPropertiesString,pricingText\n';
        const csvRows = newAIModelMetadataList.map(item => {
            return [
